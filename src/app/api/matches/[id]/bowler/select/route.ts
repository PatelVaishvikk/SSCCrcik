...
import { bowlerSelectSchema } from "@/lib/scoring/v2/schemas";
...
export async function POST(
  ...
  const parsed = bowlerSelectSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid payload", details: parsed.error.flatten() }, { status: 400 });
  }

  const { inningsNo, bowlerId, idempotencyKey, clientId, clientSeq, expectedVersion } = parsed.data;
  ...
  const validation = validateNextAction(snapshot, "BOWLER_SELECTED");
  if (!validation.ok) {
    return NextResponse.json({ error: validation.errors[0], details: validation.errors }, { status: 409 });
  }

  if (snapshot.pendingAction !== "SELECT_BOWLER") {
    return NextResponse.json({ error: "Next bowler is not required yet." }, { status: 409 });
  }

  if (!snapshot.bowlingTeamId) {
    return NextResponse.json({ error: "Bowling team not set." }, { status: 409 });
  }

  if (!isPlayerInXI(match, snapshot.bowlingTeamId, bowlerId)) {
  ...
  if (config.settings.noConsecutiveBowler && snapshot.lastOverBowlerId === bowlerId) {
    return NextResponse.json({ error: "Same bowler cannot bowl consecutive overs." }, { status: 400 });
  }
  ...
  await writeAuditLog({
    ...
    userId: session.sub,
    action: "EVENT_ADDED",
    details: { type: "BOWLER_SELECTED", seq },
  });
